...
/// <summary>
/// Checks that each difficulty's AR/OD/HP/CS falls within the catch RC per-level ranges.
///
/// RC: https://osu.ppy.sh/wiki/en/Ranking_Criteria/osu!catch — per-difficulty "Difficulty setting guidelines".
/// </summary>
public class CheckCatchDifficultyRanges : ICheck
{
    private readonly IssueTemplateSettingsOutOfRange templateOutOfRange;

    public CheckCatchDifficultyRanges()
    {
        templateOutOfRange = new IssueTemplateSettingsOutOfRange(this);
    }

    public CheckMetadata Metadata { get; } = new CheckMetadata(CheckCategory.Settings, "Difficulty settings outside ranking criteria ranges (catch)");
    ...
    public IEnumerable<Issue> Run(BeatmapVerifierContext context)
    {
        foreach (var verified in context.AllDifficulties)
        {
            ...
            double? stars = CatchStarRating.TryCalculate(verified.Working, verified.Playable.BeatmapInfo);
            if (stars == null)
                continue;

            var level = DifficultyRatingHelper.GetLevel(stars.Value);
            if (!CatchDifficultyRanges.TryGet(level, out var range))
                continue;
