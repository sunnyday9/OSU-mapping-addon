...
[TestFixture]
public class TaikoGenerationAdvancedTest
{
    ...
    {
        string tempDir = Path.Combine(Path.GetTempPath(), $"aistudio-taiko-adv-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            var result = new TaikoMapGenerator().GenerateAsync(settings).GetAwaiter().GetResult();
    ...
    public void GenerateSucceedsForAllTargetLevels()
    {
        ...
        {
            string tempDir = Path.Combine(Path.GetTempPath(), $"aistudio-taiko-lvl-{Guid.NewGuid():N}");
    ...
    [Test]
    public void SpreadPlannerWithTaikoProducesValidSpread()
    {
    ...
    {
        var upperOnly = new Dictionary<string, IReadOnlyDictionary<string, double>>
        {
            ["spacing_px"] = new Dictionary<string, double> { ["P5"] = 25, ["P95"] = 310 },
        };
        var upperSet = DistributionSet.FromDictionary(upperOnly);
        Assert.That(upperSet.SpacingPx.P5, Is.EqualTo(25));
        Assert.That(upperSet.SpacingPx.P95, Is.EqualTo(310));

        var lower = new Dictionary<string, IReadOnlyDictionary<string, double>>
        {
            ["spacing_px"] = new Dictionary<string, double> { ["p5"] = 25, ["p95"] = 310 },
        };
        var lowerSet = DistributionSet.FromDictionary(lower);
        Assert.That(lowerSet.SpacingPx.P5, Is.EqualTo(25));
        Assert.That(lowerSet.SpacingPx.P95, Is.EqualTo(310));
    }

    [Test]
    public void InMemoryWorkingBeatmapReturnsBeatmapAndHandlesOverrides()
    {
        string tempDir = Path.Combine(Path.GetTempPath(), $"aistudio-taiko-wm-{Guid.NewGuid():N}");
        string audioPath = Path.Combine(tempDir, "test_audio.mp3");
        string outputDir = Path.Combine(tempDir, "out");
        Directory.CreateDirectory(tempDir);
        ...
        try
        {
            ...
            double stars = new osu.Game.Rulesets.Taiko.Difficulty.TaikoDifficultyCalculator(new TaikoRuleset().RulesetInfo, working).Calculate().StarRating;
    ...
    public void InMemoryWorkingBeatmapDirectConstruction()
    {
        ...
        beatmap.ControlPointInfo.Add(0, new Beatmaps.ControlPoints.TimingControlPoint { BeatLength = 500 });
        beatmap.HitObjects.Add(new osu.Game.Rulesets.Taiko.Objects.Hit { StartTime = 1000, Type = osu.Game.Rulesets.Taiko.Objects.HitType.Centre });
    ...
    {
        string tempDir = Path.Combine(Path.GetTempPath(), $"aistudio-taiko-fail-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempDir);
        ...
        try
        {
            ...
            var result2 = new TaikoMapGenerator(new FakeAudioAnalyzer()).GenerateAsync(emptyAudio).GetAwaiter().GetResult();
