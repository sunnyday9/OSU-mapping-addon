...
[TestFixture]
public class CatchMapGeneratorTest
{
    ...
    {
        string tempDir = Path.Combine(Path.GetTempPath(), $"aistudio-catch-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            Assert.That(result.QualityReport!.Gates.Any(g => g.Status == GateStatus.Passed), Is.True);
        }
        finally
        {
    ...
    [Test]
    public void DecodedHasFruitObjects()
    {
        string tempDir = Path.Combine(Path.GetTempPath(), $"aistudio-catch-{Guid.NewGuid():N}");
        string audioPath = Path.Combine(tempDir, "test_audio.mp3");
        string outputDir = Path.Combine(tempDir, "out");
        ...
        try
        {
            ...
            // Check at least some catch objects; decoded type depends on converter but count should be high.
    ...
    private static GenerationResult Generate(string audioPath, string outputDir)
        => new CatchMapGenerator(new FakeAudioAnalyzer()).GenerateAsync(CreateSettings(audioPath, outputDir)).GetAwaiter().GetResult();
