...
[TestFixture]
public class CatchDifficultyRangesTest
{
    [Test]
    public void AllContainsSixLevels()
    {
        Assert.That(CatchDifficultyRanges.All.Count, Is.EqualTo(6));
        var levels = CatchDifficultyRanges.All.Select(r => r.Level).ToHashSet();
    ...
    {
        foreach (var level in Enum.GetValues<DifficultyLevel>())
            Assert.That(CatchDifficultyRanges.TryGet(level, out var range), Is.True, $"{level} TryGet");
    }
    ...
    public void GetReturnsCorrectLevel()
    {
        foreach (var level in Enum.GetValues<DifficultyLevel>())
        {
            var range = CatchDifficultyRanges.Get(level);
    ...
    public void TryGetReturnsFalseForInvalidEnum()
    {
        ...
        Assert.That(CatchDifficultyRanges.TryGet(invalid, out _), Is.False);
        Assert.That(() => CatchDifficultyRanges.Get(invalid), Throws.TypeOf<InvalidOperationException>());
    }

    [Test]
    public void AllRangesHaveValidBounds()
    {
        foreach (var range in CatchDifficultyRanges.All)
        {
    ...
    {
        var expert = CatchDifficultyRanges.Get(DifficultyLevel.Expert);
        var expertPlus = CatchDifficultyRanges.Get(DifficultyLevel.ExpertPlus);
    ...
    {
        var easy = CatchDifficultyRanges.Get(DifficultyLevel.Easy);
        Assert.That(easy.ApproachRate.Min, Is.EqualTo(0));
        Assert.That(easy.ApproachRate.Max, Is.EqualTo(5));
        Assert.That(easy.OverallDifficulty.Min, Is.EqualTo(1));
        Assert.That(easy.OverallDifficulty.Max, Is.EqualTo(3));
        Assert.That(easy.HpDrain.Min, Is.EqualTo(1));
        Assert.That(easy.HpDrain.Max, Is.EqualTo(3));
    ...
    public void ExpertRangesMatchExpected()
    {
        ...
        Assert.That(expert.OverallDifficulty.Min, Is.EqualTo(8));
    ...
    public void ContainsLogicForEasyAndExpertPlus()
    {
        ...
        Assert.That(easy.Contains(easyAr, easyOd, 10, easyCs), Is.False);
        Assert.That(easy.Contains(easyAr, easyOd, easyHp, 10), Is.False);
    ...
    {
        var range = CatchDifficultyRanges.Get(DifficultyLevel.Hard);
