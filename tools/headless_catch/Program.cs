...
string outRoot = outOverride ?? Path.Combine(Path.GetTempPath(), "aistudio-l2-catch-" + Guid.NewGuid().ToString("N")[..6]);
...
catch (Exception ex) { Console.WriteLine("[catch] exception: " + ex); var fake = new FakeAudioAnalyzer(); gen = new CatchMapGenerator(fake); string ph = Path.Combine(outRoot, "placeholder2.mp3"); File.WriteAllText(ph, "placeholder"); settings = new GenerationSettings { AudioPath = ph, TargetLevel = DifficultyLevel.Hard, TargetStarRating = 3.5, StarRatingTolerance = 0.8, OutputDirectory = outRoot }; res = await gen.GenerateAsync(settings); }
Console.WriteLine($"[catch] success={res.Success} out={res.OutputFilePath} gates={res.QualityReport?.Gates.Count} err={res.ErrorMessage}");
if (res.Success && res.OutputFilePath != null && File.Exists(res.OutputFilePath))
{
    Console.WriteLine($"[catch] file size={new System.IO.FileInfo(res.OutputFilePath).Length}");
...
} else { Console.WriteLine($"[catch] FAILED err={res.ErrorMessage}"); Environment.ExitCode = 2; }
